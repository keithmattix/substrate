...
func NewRouterCmd() *cobra.Command {
	...
	cmd.Flags().IntVar(&cfg.ConnectPort, "port-connect", 8081, "TCP port for CONNECT-tunneled traffic entering through the ate-router Router")
	cmd.Flags().IntVar(&cfg.ConnectTLSPort, "port-connect-tls", 8443, "TCP port for CONNECT-tunneled traffic entering through the ate-router Router over TLS")
	...
	cmd.Flags().IntVar(&cfg.ExtProcMaxConnections, "extproc-max-connections", 512, "Circuit-breaker max_connections for the network ext proc cluster; the default is 512.")
