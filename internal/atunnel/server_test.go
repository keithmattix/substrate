...
func TestMutualTLSClientIdentity(t *testing.T) {
	...
	s, err := NewServer(Config{
		...
		AllowedClientID:      "spiffe://cluster.local/ns/ate-system/sa/ateway-ingress",
		Upstream:             upstream,
	})
	if err != nil {
		t.Fatal(err)
	...
	tests := []struct {
	...
	}{
		{
			...
			cert: ca.issue(t, "spiffe://cluster.local/ns/ate-system/sa/ateway-ingress", []x509.ExtKeyUsage{x509.ExtKeyUsageClientAuth}),
		},
		{
			name:    "wrong identity",
		...
		},
		{
			...
			cert:    untrustedCA.issue(t, "spiffe://cluster.local/ns/ate-system/sa/ateway-ingress", []x509.ExtKeyUsage{x509.ExtKeyUsageClientAuth}),
			wantErr: true,
		},
	}
	for _, tt := range tests {
